...
<script type="text/html" id="barDemo">
  ...
  <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">在职</a>
</script>
	...
	layui.use(['table','form'], function(){
		...
		$.ajax({
			...
			success:function(data){//请求成功后的事件
					$.each(data.data,function(index,obj){
						...
						$("#tPosition").append(option);	form.render(); 
					})
				}
			})
		...
		table.render({
		    ...
		    ,height: 471
		    ,page:true
	    ,url: 'Teacher?action=select&zw=离职' //数据接口
	    ,cols: [[ //表头
	  ...
	  ,id:"idsw"
		 ...
		 form.on('select(tPosition)', function(data){
			  table.render({
				    ...
				    ,height: 471
				    ,page:false
				    ,url: 'Teacher?action=PagingtPosition&zw=离职&tPosition='+data.value //数据接口
				    ,cellMinWidth: 120
				    ,cols: [[ //表头
		 ...
		 //监听工具条
		  table.on('tool(demo)', function(obj){
		    ...
		    if(obj.event === 'detail'){
		      ...
		      layer.confirm('真的在职么', function(index){
		    	  del(data.tid);
		      });
...
<script>
	$("#snamesele").click(function(){
		 table.render({
			    ...
			    ,height: 471
			    ,page:false
			    ,url: 'Teacher?action=selectsname&zw=离职&tname='+$("#demoReload").val() //数据接口
			    ,cellMinWidth: 120
			    ,cols: [[ //表头
